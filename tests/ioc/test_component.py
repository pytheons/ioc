...
class ParameterResolverTest(unittest.TestCase):
    ...
    def test_parameters_strings(self):
        ...
        self.assertEqual(['hello world', 'hello world'],
                         parameter_resolver.resolve(["%bonjour% %le_monde%", "%bonjour% %le_monde%"], holder))

    def test_parameters_numbers_booleans(self):
        holder = ioc.component.ParameterHolder()
        parameter_resolver = ioc.component.ParameterResolver()
    ...
    def test_parameter_env_system_is_none(self):
        EnvironmentVariables.clear('SYSTEM')
        holder = ioc.component.ParameterHolder()
        holder.set('my.env.paramter', '%env(SYSTEM)%')
        parameter_resolver = ioc.component.ParameterResolver()

        self.assertEqual(None, parameter_resolver.resolve(holder.get('my.env.paramter'), holder))

    def test_parameter_env_system_is_empty_string(self):
        os.environ['SYSTEM'] = ''
        holder = ioc.component.ParameterHolder()
        holder.set('my.env.paramter', '%env(SYSTEM)%')
        parameter_resolver = ioc.component.ParameterResolver()

        self.assertEqual('', parameter_resolver.resolve(holder.get('my.env.paramter'), holder))

    def test_parameter_env_system_is_empty(self):
        os.environ['SYSTEM'] = 'SomeString'
        holder = ioc.component.ParameterHolder()
        holder.set('my.env.paramter', '%env(SYSTEM)%')
        parameter_resolver = ioc.component.ParameterResolver()

        self.assertEqual('SomeString', parameter_resolver.resolve(holder.get('my.env.paramter'), holder))

    def test_parameter_env_system_cast_to_int(self):
        os.environ['SYSTEM'] = '1'
        holder = ioc.component.ParameterHolder()
        holder.set('my.env.paramter', '%env(int:SYSTEM)%')
        parameter_resolver = ioc.component.ParameterResolver()

        self.assertEqual(1, parameter_resolver.resolve(holder.get('my.env.paramter'), holder))

    def test_parameter_env_system_cast_to_string(self):
        os.environ['SYSTEM'] = 'string'
        holder = ioc.component.ParameterHolder()
        holder.set('my.env.paramter', '%env(str:SYSTEM)%')
        parameter_resolver = ioc.component.ParameterResolver()

        parameter = parameter_resolver.resolve(holder.get('my.env.paramter'), holder)
        self.assertIsInstance(parameter, str)
        self.assertEqual('string', parameter_resolver.resolve(holder.get('my.env.paramter'), holder))

    def test_parameter_env_system_cast_to_trimmed_string(self):
        os.environ['SYSTEM'] = ' value '
        holder = ioc.component.ParameterHolder()
        holder.set('my.env.paramter', '%env(trim:SYSTEM)%')
        parameter_resolver = ioc.component.ParameterResolver()

        parameter = parameter_resolver.resolve(holder.get('my.env.paramter'), holder)
        self.assertIsInstance(parameter, str)
        self.assertEqual('value', parameter_resolver.resolve(holder.get('my.env.paramter'), holder))

    def test_parameter_env_system_cast_to_json(self):
        os.environ['SYSTEM'] = '{"A":"B"}'
        holder = ioc.component.ParameterHolder()
        holder.set('my.env.paramter', '%env(json:SYSTEM)%')
        parameter_resolver = ioc.component.ParameterResolver()

        parameter = parameter_resolver.resolve(holder.get('my.env.paramter'), holder)
        self.assertIsInstance(parameter, dict)
        self.assertEqual({"A": "B"}, parameter_resolver.resolve(holder.get('my.env.paramter'), holder))
...
class ContainerBuilderTest(unittest.TestCase):
    ...
    def test_get_class(self):
        ...
        self.assertEqual(c.__name__, tests.ioc.service.Fake.__name__)
    ...
    def test_get_container(self):
        self.container.add('service.id.1',
                           ioc.component.Definition('tests.ioc.service.Fake', [True], {'param': 'salut'}))
        self.container.add('service.id.2',
                           ioc.component.Definition('tests.ioc.service.Fake', [False], {'param': 'hello'}))
        self.container.add('service.id.3', ioc.component.Definition('tests.ioc.service.Foo',
                                                                    [ioc.component.Reference('service.id.2'), None]))
    ...
    def test_cyclic_reference(self):
        self.container.add('service.id.1', ioc.component.Definition('tests.ioc.service.Foo',
                                                                    [ioc.component.Reference('service.id.1'), None]))
    ...
    def test_reference_with_method(self):
        self.container.add('service.id.1', ioc.component.Definition('tests.ioc.service.Fake', [
            ioc.component.Reference('service.id.2', 'set_ok')]))
    ...
    def test_create_definition_from_abstract_definition(self):
        self.container.add('service.id.abstract',
                           ioc.component.Definition('tests.ioc.service.Fake', ['foo'], abstract=True))
